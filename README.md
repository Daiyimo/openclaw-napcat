...
*   **表情回应 (Reaction)**：收到触发消息时，自动对消息添加表情回应，提升交互体验。支持 `auto` 智能模式，采用混合两层机制：
    *   **第一层（本地即时）**：`pickLocalEmoji()` 根据消息语义秒回对应表情——搞笑贴😂、夸赞贴👍、难过贴😭、任务贴👌等 12 种分类
    *   **第二层（AI 补充）**：AI 根据语义在回复中输出 `[reaction:ID]` 标记，额外贴情感 emoji（与本地不同时才贴，防重复）
    *   **纯问候/感谢**：不贴表情，正常文字回复
...
根据消息内容关键词匹配，秒回对应语义的表情（基于 [QQ 官方表情对象](https://bot.q.qq.com/wiki/develop/api-v2/openapi/emoji/model.html)）：

| 消息内容 | 表情 | ID |
| :--- | :--- | :--- |
| 问模型/你是谁/什么ai/哪家模型/谁开发的 | 🐱 喵喵 | 307 |
| 难过/伤心/崩溃/破防/emo/泪目 | 😭 大哭 | 128557 |
| 哈哈/笑死/搞笑/233/xswl/笑不活 | 😂 激动 | 128514 |
| 厉害/牛/666/nb/yyds/绝绝子 | 👍 厉害 | 128077 |
| 卧槽/绝了/离谱/服了/裂开/omg | 🔥 火 | 128293 |
| 加油/冲/fighting/干巴爹/冲冲冲 | 💪 肌肉 | 128170 |
| 喜欢/爱你/比心/心动/恋爱 | 💓 爱心 | 128147 |
| 恭喜/太好了/成功/发财/撒花/万岁 | 🎉 庆祝 | 127881 |
| 嘿嘿/可爱/卖萌/撒娇/嘻嘻 | 😊 嘿嘿 | 128522 |
| 困了/好累/摸鱼/摆烂/躺平 | 💤 睡觉 | 128164 |
| 好吃/干饭/馋/真香/好饿 | 🍻 干杯 | 127867 |
| 好看/漂亮/帅/颜值/好美 | ✨ 闪光 | 10024 |
| 好奇/有趣/无聊/陪我/怎么回事/啥意思 | 🐱 喵喵 | 307 |
| 疑问句（?？吗呢吧么结尾） | 👌 好的 | 128076 |
| 任务请求/斜杠命令/含URL | 👌 好的 | 128076 |
| 其他所有内容（默认） | 🐱 喵喵 | 307 |
...
AI 回复中若包含 `[reaction:ID]` 标记，且与本地已贴的表情不同，则额外贴一个情感表情：

| AI 输出 | 行为 |
| :--- | :--- |
| `[reaction:128514]哈哈` | 本地已贴 😂 则跳过（相同），贴了其他则额外贴 😂 |
| `[task:ok]好的` | 本地已贴则跳过，未贴则补贴 👌 |
| 无标记 | 不额外操作 |
...
| 文件 | 变更类型 | 说明 |
| :--- | :--- | :--- |
| `src/channel.ts` | 修复 + 重构 | 修复 2 个 Bug，新增 `pickLocalEmoji()` 智能语义匹配（14 种分类 + 喵喵默认），本地即时贴表情 + AI `[reaction:ID]` 补充，更新系统提示词 |
...
| 群聊发 "卧槽绝了" | 🔥 | 震惊 → 火 |
| 群聊发 "好好看" | ✨ | 颜值 → 闪光 |
| 群聊发 "好无聊啊" | 🐱 喵喵 | 好奇/闲聊 → 喵喵 |
| 群聊发 "今天天气不错" | 🐱 喵喵 | 未匹配其他 → 默认喵喵 |
| 群聊发 "你好" | 无 | 纯问候排除 |
| 群聊发 "谢谢" | 无 | 纯感谢排除 |
| 回复消息发 `/setessence` | — | 不再报错，正常设精华 |
| 私聊发 `/cache` | — | 在私聊回复"缓存已清理" |
