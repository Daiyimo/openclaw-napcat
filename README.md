...
*   **表情回应 (Reaction)**：收到触发消息时，自动对消息添加表情回应（如竖起大拇指），提升交互体验。支持 `auto` 智能模式，采用混合两层机制：
    *   **第一层（本地即时）**：除纯问候/感谢外，所有消息立即贴 OK emoji 👌，毫秒级响应
    *   **第二层（AI 补充）**：AI 根据语义在回复中输出 `[reaction:ID]` 标记，额外贴情感 emoji（👍👏🔥😄😂等）
    *   **一般对话**（纯问候、纯感谢）：不贴表情，正常文字回复
...
### v1.4.2 - Bug修复与混合表情回应 (2025-02-24) — update by Claude-4.6-Opus

修复多个已知 Bug，采用混合模式实现表情回应：本地即时贴 OK 表情保底 + AI 补充情感表情。

#### Bug 修复

| Bug | 说明 |
| :--- | :--- |
| **`replyMsgId` 变量声明顺序错误** | `replyMsgId` 在管理员命令块中使用时尚未声明，导致 `/setessence` 和 `/delessence` 命令触发 ReferenceError。现已将声明移至管理员命令块之前。 |
| **`/cache` 命令私聊报错** | `/cache` 始终使用 `sendGroupMsg` 发送响应，在私聊中调用会报错。现已改为根据场景自动选择 `sendGroupMsg` 或 `sendPrivateMsg`。 |
| **`setMsgEmojiLike` 调用方式修复** | 原方法使用 `sendWs`（fire-and-forget），改为 `sendAction`（HTTP 优先、WS 降级），并确保 `message_id` 为数字类型；所有调用点补充 `await`。 |

#### 混合表情回应架构

采用两层机制，兼顾响应速度和智能表情选择：

**第一层：本地即时检测（毫秒级）**

`isTaskLikeMessage()` 使用宽松的排除法——只有纯问候和纯感谢不贴表情，其余消息立即贴 OK 👌 表情：

| 排除规则（不贴表情） | 示例 |
| :--- | :--- |
| 纯问候语 | 你好、hello、hi、在吗、早安、晚安、哈喽… |
| 纯感谢语 | 谢谢、感谢、多谢、thanks、thank you… |

所有其他消息类型均立即贴 OK 表情，覆盖以下场景：
- 中文请求动词：帮我/查询/查找/查看/翻译/设置/打开/关闭/发送/提醒/计算/搜索/下载/上传/生成/创建/删除/修改/更新/运行/执行/分析/总结/整理/推荐/对比/比较/转发/获取
- 中文疑问词：怎么办/如何/什么是/多少/为什么/怎么/哪里/几点/几号/有没有/能否/是否/可不可以
- 问号/语气词结尾：?？吗呢吧么
- 包含 URL：http:// 或 https://（发链接通常是任务请求）
- 英文指令：help/please/can you/how to/translate/search/find
- 斜杠命令：/help、/status 等
- 闲聊/情感/吐槽/搞笑等一切非问候非感谢内容

**第二层：AI 补充情感表情（2-5 秒后）**

AI 回复中若包含 `[reaction:ID]` 标记，额外贴一个情感表情，与本地 OK 表情共存：

| AI 输出 | 行为 |
| :--- | :--- |
| `[reaction:128514]哈哈` | 额外贴 😂 表情（OK 表情已由本地贴过） |
| `[task:ok]好的` | 本地已贴 OK 则跳过，未贴则补贴 |
| 无标记 | 不额外操作 |
...
| 文件 | 变更类型 | 说明 |
| :--- | :--- | :--- |
| `src/channel.ts` | 修复 + 重构 | 修复 2 个 Bug，新增 `isTaskLikeMessage()` 宽松检测，本地即时贴 OK 表情 + AI `[reaction:ID]` 补充，更新系统提示词 |
| `src/client.ts` | 修复 | `setMsgEmojiLike` 改用 `sendAction` + `Number(messageId)` |
| `README.md` | 文档 | 同步更新日志 |
...
| 场景 | 预期结果 |
| :--- | :--- |
| 群聊发 "帮我查天气" | 立即贴 OK 表情，AI 正常回复 |
| 群聊发 "查询电费 http://..." | 立即贴 OK 表情（URL 命中），AI 正常回复 |
| 群聊发 "哈哈太搞笑了" | 立即贴 OK 表情，AI 若输出 `[reaction:128514]` 则额外贴 😂 |
| 群聊发 "你好" | 不贴表情（纯问候排除），AI 正常回复 |
| 群聊发 "谢谢" | 不贴表情（纯感谢排除），AI 正常回复 |
| 回复消息发 `/setessence` | 不再报错，正常设精华 |
| 私聊发 `/cache` | 在私聊回复"缓存已清理"，不报群消息错误 |

### v1.4.1 - 智能Emoji回复优化 (2026-02-23)

优化了 `reactionEmoji: "auto"` 模式的语义识别逻辑，提升交互体验。

#### 优化内容

| 功能 | 说明 |
| :--- | :--- |
| **智能emoji回复** | 当 `reactionEmoji` 设为 `"auto"` 时，AI会根据消息语义自动选择合适的emoji反应 |
| **任务类请求** | 识别为查询、提醒、执行操作等任务请求时，先在消息上添加 OK emoji 👌，再发送文字回复 |
| **闲聊情感类** | 识别为闲聊、情感表达时，根据语义选择 👍👏🔥😄😂 等emoji，再发送文字 |
| **一般对话** | 问候、感谢等普通对话正常文字回复，不添加emoji |

#### 涉及文件

| 文件 | 变更类型 | 说明 |
| :--- | :--- | :--- |
| `src/channel.ts` | 优化 | 新增 `[task:emoji_only]` 标记处理，增强语义识别prompt |
| `README.md` | 文档 | 同步智能emoji回复功能说明 |
