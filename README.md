...
*   **表情回应 (Reaction)**：收到触发消息时，自动对消息添加表情回应（如竖起大拇指），提升交互体验。支持 `auto` 智能模式，根据消息语义自动选择合适的emoji：
    *   **任务类请求**（查询、提醒、执行操作等）：本地即时检测并秒回 OK emoji 👌，无需等待 AI 响应
    *   **闲聊/情感类消息**：由 AI 根据语义选择合适的 emoji（👍👏🔥😄😂等），再发送文字回复
    *   **一般对话**（问候、感谢等）：正常文字回复
...
### v1.4.2 - Bug修复与本地任务表情优化 (2025-02-24) — update by Claude-4.6-Opus

修复多个已知 Bug，并将任务类消息的 OK 表情确认从 AI 判断改为本地即时检测，大幅降低响应延迟。
...
| 功能 | 说明 |
| :--- | :--- |
| **本地任务消息检测 `isTaskLikeMessage()`** | 新增本地关键词/模式匹配函数，无需等待 AI 即可判断消息是否为任务类请求 |
| **任务消息即时 OK 表情** | 检测到任务类消息后立即发送 OK 表情 (128076)，无需等待 AI 响应，体感延迟从数秒降至毫秒级 |

#### 检测规则 (`isTaskLikeMessage`)

| 模式 | 示例 |
| :--- | :--- |
| `/` 开头的命令 | `/help`、`/status` |
| 中文请求词开头 | 帮我、请帮、能不能、查一下、翻译、设置… |
| 中文疑问词 | 怎么办、如何、什么是、多少、为什么… |
| 以 `?？吗呢吧么` 结尾 | 这是什么？、可以吗、行不行呢 |
| 英文指令模式 | help、please、can you、how to、translate… |

#### 优化内容

| 优化 | 说明 |
| :--- | :--- |
| **AI 提示词精简** | 移除 `[task:emoji_only]` 指令（已由本地处理），移除 128076(OK) 从可用表情列表，AI 仅负责闲聊/情感类 `[reaction:ID]` |
| **回复标记解析重写** | 修正 `if/else` 链为正确的互斥逻辑：`taskEmojiOnlyMatch` → `taskMatch` → `reactionMatch`，利用 `taskEmojiAlreadySent` 标志防止重复发送 |
| **向后兼容** | 保留对 `[task:emoji_only]` 和 `[task:ok]` 标记的解析，万一 AI 仍然输出也能正确处理 |

#### 涉及文件

| 文件 | 变更类型 | 说明 |
| :--- | :--- | :--- |
| `src/channel.ts` | 修复 + 优化 | 修复 2 个 Bug，新增 `isTaskLikeMessage()` 函数，优化表情发送和标记解析逻辑，精简 AI 提示词 |
| `README.md` | 文档 | 同步更新日志 |
...
| 场景 | 预期结果 |
| :--- | :--- |
| 群聊发 "帮我查天气" | OK 表情秒回，AI 文字回复不带标记 |
| 群聊发 "哈哈太搞笑了" | 无立即表情，AI 回复带 `[reaction:128514]` 自动贴上 |
