...
*   **表情回应 (Reaction)**：收到触发消息时，自动对消息添加表情回应（如竖起大拇指），提升交互体验。支持 `auto` 智能模式，完全由 AI 模型判断消息类型并选择表情：
    *   **任务类请求**（查询、提醒、执行操作等）：AI 输出 `[task:ok]` 标记，在消息上贴 OK emoji 👌
    *   **闲聊/情感类消息**：AI 根据语义选择合适的 emoji（👍👏🔥😄😂等），在消息上贴对应表情
    *   **一般对话**（问候、感谢等）：正常文字回复，不贴表情
...
### v1.4.2 - Bug修复与纯AI表情回应 (2025-02-24) — update by Claude-4.6-Opus

修复多个已知 Bug，重构表情回应为完全 AI 驱动模式，由接入的模型智能判断消息类型并决定表情回应。
...
移除本地 `isTaskLikeMessage()` 硬编码检测函数，所有表情回应判断完全交由接入的 AI 模型决定：

| 消息类型 | AI 输出 | 机器人行为 |
| :--- | :--- | :--- |
| 任务类请求（查询、提醒、执行操作等） | `[task:ok]好的，我来帮你查` | 在原消息上贴 OK 👌 表情回应，发送文字 |
| 闲聊/情感类 | `[reaction:128514]哈哈太搞笑了` | 在原消息上贴 😂 表情回应，发送文字 |
| 一般对话（问候、感谢等） | `在的，有什么可以帮你的吗？` | 仅发送文字，无表情回应 |

#### 标记解析逻辑

回复解析采用互斥 `if/else` 链，AI 回复开头只会匹配一个标记：

```
[task:emoji_only] → 贴 OK 表情 (128076)
[task:ok]         → 贴 OK 表情 (128076)
[reaction:ID]     → 贴 AI 选择的表情
无标记            → 不贴表情
```
...
| 文件 | 变更类型 | 说明 |
| :--- | :--- | :--- |
| `src/channel.ts` | 修复 + 重构 | 修复 2 个 Bug，移除本地检测函数，表情回应完全由 AI 驱动，更新系统提示词 |
...
| 场景 | 预期结果 |
| :--- | :--- |
| 群聊发 "帮我查天气" | AI 回复带 `[task:ok]`，原消息贴 OK 表情 |
| 群聊发 "哈哈太搞笑了" | AI 回复带 `[reaction:128514]`，原消息贴 😂 表情 |
| 群聊发 "查询电费 http://..." | AI 回复带 `[task:ok]`，原消息贴 OK 表情 |
| 回复消息发 `/setessence` | 不再报错，正常设精华 |
| 私聊发 `/cache` | 在私聊回复"缓存已清理"，不报群消息错误 |
| 发 "你好" | AI 不带标记，不触发任何表情 |
