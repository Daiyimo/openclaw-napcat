...
Write-Host "  4) StepFun step-3.5-flash-2603（按量计费，需要官方 API Key）" -ForegroundColor Green
Write-Host ""

do {
    $CHOICE = Read-Host "请输入数字选择 [1/2/3/4]"
...
elseif ($CHOICE -eq "3") {
    ...
    Write-Host "已选择：StepFun step-3.5-flash-2603" -ForegroundColor Green
    Write-Host ""
    ...
    try {
        ...
        # 使用 Add-Member -Force 确保可以设置或替换
        $config.models.providers | Add-Member -MemberType NoteProperty -Name "stepfun-2603" -Value $stepfunConfig -Force
        ...
        # 设置默认模型（使用 -Force）
        $config.agents.defaults.model | Add-Member -MemberType NoteProperty -Name "primary" -Value "stepfun-2603/step-3.5-flash-2603" -Force
        ...
        Write-Host "  配置更新完成!" -ForegroundColor Green
        Write-Host "==========================================" -ForegroundColor Green
        Write-Host "  - 已添加 StepFun step-3.5-flash-2603" -ForegroundColor White
        Write-Host "  - 默认模型: stepfun-2603/step-3.5-flash-2603" -ForegroundColor White
        Write-Host "  - API 地址: https://api.stepfun.com/v1" -ForegroundColor White
        Write-Host "  - 备份文件: $backupFile" -ForegroundColor White
    }
    catch {
        Write-Host "配置文件处理失败: $_" -ForegroundColor Red
