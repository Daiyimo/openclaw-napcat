...
export const QQConfigSchema = z.object({
  ...
  botSignatureStyle: z.enum(["none", "visible", "zero-width", "metadata"]).optional().default("metadata").describe("Bot identification strategy: 'metadata' (default) uses out-of-band OneBot json segments — clean user text, requires other openclaw-napcat bots >=v1.8. 'visible' appends [BOT:selfId] to message text (reliable, user-visible). 'zero-width' uses invisible Unicode characters (clean UX, may be stripped by some platforms). 'none' disables text signatures entirely (relies solely on sender.bot, knownBotIds whitelist, and persistent known-bots cache)."),
