...
export const qqChannel: ChannelPlugin<ResolvedQQAccount> = {
  ...
  messaging: {
    ...
    resolveOutboundSessionRoute: async (params: {
      ...
      const trimmed = params.target.replace(/^napcat:/i, "").trim();
      if (!trimmed) return null;
      let peerKind: "direct" | "group" | "channel" = "direct";
      let peerId = trimmed;
      if (trimmed.startsWith("group:")) { peerKind = "group"; peerId = trimmed.slice(6); }
      else if (trimmed.startsWith("channel:")) { peerKind = "channel"; peerId = trimmed.slice(8); }
      else if (trimmed.startsWith("guild:")) { peerKind = "channel"; peerId = trimmed.slice(6); }
      else if (trimmed.startsWith("private:")) { peerKind = "direct"; peerId = trimmed.slice(8); }
      if (!peerId) return null;
      const peerIdNorm = peerId.toLowerCase();
      const sessionKey = `agent:${params.agentId}:napcat:${peerKind}:${peerIdNorm}`;
      return {
        sessionKey,
